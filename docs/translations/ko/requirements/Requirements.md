...
- *quantified* 보정 노출은 숫자 primary 행 + status 배지 표시. status 카테고리는 *No correction* (source의 무보정 임계치 내부), *Formula-derived* / *Custom formula* 또는 *Table-derived* / *Custom table* (활성 계산 곡선 위 — guarded formula 또는 log-log table interpolation; table/formula는 official 제조사 source일 수도, 사용자 작성 custom profile일 수도 있으며 계산 basis는 동일), *Beyond source range* / *Outside guidance* (formula 또는 table이 source의 supported boundary 너머에서 numeric continuation 산출; 배지는 warning tone) 중 하나.
...
2. timer가 shooting 화면의 calculator 옆에 계속 보이는 *compact dock*에 나타난다. 사용자는 timer를 닫거나 시야에서 잃지 않으면서 다음 컷을 위해 ND를 조정하거나 필름을 바꿀 수 있다.
3. compact dock은 각 실행 중인 timer에 대해 다음을 전달: 남은 시간, 여러 시간 스케일에 걸친 *진행감* (30 s timer와 30 min timer 모두 반응적으로 느껴지도록), 이름과 시간 텍스트와 무관한 식별 단서.
4. 사용자는 compact dock에서 *full-screen Timers workspace* — 모든 실행/일시정지/완료/canceled timer를 함께 보여주는 별도 화면 — 를 열 수 있다. 정확한 배치/진입 제스처는 디자인 결정이며 요구사항이 아니다.
5. timer의 duration이 만료되면, 시스템은 사용자가 폰을 보고 있지 않더라도 (카메라 들고 있거나 폰이 주머니에 있거나) 도달하는 완료 신호를 표시하고, 앱 안 timer 카드는 "Done" 상태로 전환.

**경계 조건.**
- 각 timer는 안정된 식별자를 가짐. 정렬: active 그룹은 가장 최근 시작 순, terminal 그룹(completed와 canceled 함께)은 가장 최근 terminal 시각 순.
...
**단계.**
1. 시나리오 3을 반복해 두 번째 timer를 시작. compact dock은 추가 timer를 수용하면서도 위쪽 calculator를 잃지 않으며, 각 timer는 텍스트를 읽지 않고도 카드를 한눈에 식별할 수 있는 고유 식별 단서를 포함한다.
2. 단일 timer에 focus 해 더 자세히 검사 가능.
3. 다른 timer가 계속 실행되는 동안 한 timer만 일시정지 가능.
4. Completed와 canceled timer는 함께 terminal 그룹에 모이고 가장 최근 terminal 시각 순.

**경계 조건.**
- timer의 식별 단서는 *해당 timer의 유지되는 동안 안정* — 사용자가 재정렬하거나 focus 하거나 active 그룹과 terminal (completed/canceled) 그룹 사이로 옮겨도 변하지 않는다.
- compact dock과 full-screen Timers workspace는 각각 동일한 underlying timer collection에서 카드를 표시; full-screen workspace를 열거나 닫아도 compact dock의 카드를 재계산하지 않는다.
...
4. running 또는 paused timer가 하나도 남지 않으면 (각각 completed, canceled, 또는 제거되어) 잠금 화면 표면이 사라진다.
...
- **FR-1.2** 사용자는 ND 값을 shipping ND ladder — whole stop `0, 1, 2, …, 30` 및 세 개의 고정 fractional commercial preset — 에서만 입력. One-third-stop은 base shutter 에만 적용; ND ladder는 해당 preset을 제외하면 모든 출시 모드에서 whole-stop 유지. (시나리오 1)
- **FR-1.2a** 사용자는 최대 네 개의 ND filter wheel을 stack 할 수 있으며, 각 wheel은 shipping ND ladder에서 선택. 시스템은 stack의 합을 유효 ND 값으로 계산하며 그 합이 구조적으로 30 stop을 넘지 않도록 유지 — 각 wheel의 선택 가능 범위는 남은 예산만큼 줄어들고, cap을 넘는 선택은 clamp 되지 않고 거부된다. 변경이 확정되면 wheel들은 값 내림차순으로 (0은 마지막) 스스로 정렬하되 wheel별 identity는 유지. (시나리오 1)
...
- **FR-2.1** 시스템은 launch 시점에 큐레이트된 preset 필름 set을 제공 — 각 필름은 정확히 하나의 primary reciprocity profile 보유. Launch 카탈로그는 주로 current official 제조사 문서에서 source 되어야 하며 quantified profile — guarded-formula profile과 official table log-log profile (둘 다 옵션 제조사 source-evidence row 포함) — 그리고 장노출 영역이 정성적인 limited-guidance profile 모두를 cover. 정량 안내가 verified third-party 출처뿐인 필름은 대신 honestly-labeled unofficial-authority primary profile로 ship 될 수 있다 (reciprocity/catalog.md 참조); 이 class는 현 release에 존재한다. 출판된 안내 — threshold 범위, color-filter 권고, 현상 시간 hint, stop-signal 경계 — 를 사용자가 drill into 할 수 있는 형태로 보존. (시나리오 2)
...
- **FR-2.8** Reciprocity 커버리지는 *정량 공식*이 있는 필름으로 한정되지 않는다. threshold-only / limited-guidance 출판 가이드는 일차 지원 범위로 다루며 (보조 대체이 아님). Honestly-labeled unofficial-authority preset entry (FR-2.1)와 user-defined custom entry (FR-2.9, FR-2.15)는 official 제조사 profile과 나란히 현재 일차 지원 범위다 — 둘 다 예약된 향후 capacity가 아니다. (시나리오 2 경계; FR-2.2 보완)
...
- **FR-2.15** 사용자는 metered→corrected duration anchor row를 입력해 **custom reciprocity table profile**을 작성할 수 있다; table은 preset table profile과 동일한 log-log interpolation model로 계산된다. Custom table profile은 custom formula profile과 동일한 save/reuse/select/edit/delete lifecycle을 지원 (FR-2.10 확장). (Persona 1.2)
- **FR-2.16** 저장된 custom table로부터 시스템은 inspection 전용 fitted-formula preview를 derive 한다 — 이는 실제 shooting 계산으로 쓰이지 않는다. Preview는 fit/error 정보를 포함; range 어디서든 exposure를 shorten 하는 fit은 채택 경로 없이 unusable로 표시되고, 단순히 poor한 fit은 inspection을 막지 않으면서 warning을 표시. (Persona 1.2)
- **FR-2.17** 사용자는 저장된 table의 fitted preview로부터 seed 된 새롭고 독립적인 **custom Formula** profile을 생성할 수 있다 ("Create Formula from table"). Cancel은 아무것도 생성하지 않고; Save는 source table과 별개의 profile을 생성하며 이후 계산 parameter는 table과 독립 — 어느 한쪽을 편집/삭제해도 다른 쪽은 변하지 않는다. (Persona 1.2; FR-2.9, FR-2.10 확장)
- **FR-2.18** preset 필름은 하나 이상의 선택 가능한 reciprocity profile을 노출할 수 있다 — primary 하나와 alternate/derived model (예: official table과 나란한 app-derived formula). 둘 이상을 제공하는 필름의 경우 사용자는 그중에서 선택할 수 있고, 선택은 카메라 슬롯별로 앱 재시작을 거쳐도 유지 (FR-8.2, FR-5.2 확장). (Persona 1.2, 1.3; 시나리오 2)

### 3.3 Timer 라이프사이클

- **FR-3.1** 시스템은 *엄격히 양수이고 유한한* 인 duration 만으로 timer를 시작. 무한, NaN, non-positive duration은 진입점에서 거부 — 어떤 영속 상태도 쓰기 전. (시나리오 3 경계)
- **FR-3.2** Timer는 다음 상태 전환만 따라 이동: *running → paused*, *paused → running*, *running → completed*, *running → canceled*, *paused → canceled*. *Paused → completed*는 direct transition이 아니다 — paused timer는 completed 되기 전 반드시 resume을 거쳐야 한다; 남은시간이 이미 0인 상태에서 pause를 시도하면 paused로 진입하는 대신 곧바로 completed로 short-circuit 한다 (FR-3.5). 다른 전환은 표현 불가능; *completed*와 *canceled*는 둘 다 terminal이며 서로 구분 — canceled timer는 절대 completed가 되지 않는다. (시나리오 3, 4)
...
- **FR-3.7** Active timer는 가장 최근 시작 순으로 표시; completed와 canceled timer는 함께 하나의 terminal 그룹을 이루어 가장 최근 terminal 시각 순으로 표시. 동률은 결정적으로 해소되어 사용자가 불안정한 순서를 보지 않는다. (시나리오 5)

### 3.4 Multi-timer + 잠금 화면

- **FR-4.1** 시스템은 다중 동시 timer를 지원하며, 각 timer는 running / paused / completed / canceled / 재정렬 / focus / inspect 전환을 가로질러 살아남는 안정된 식별자를 가진다. (시나리오 5)
...
- **FR-5.1** Timer 상태 (state machine에 필요한 running / paused / completed / canceled 정보)와 timer 표시 metadata (사용자가 보는 이름, basis-summary 행, LIFO 삽입 순서) 모두 앱 재시작 후에도 유지된다. (시나리오 7)
- **FR-5.2** Calculator 컨텍스트 — 선택된 필름, 노출 scale 토큰, base shutter, ND filter stack (FR-1.2a; 통째로 복원되며, 무효한 영속화 stack은 legacy single ND 값으로 통째로 되돌아가고 clamp 되지 않는다), 그리고 설정된 경우 Target Shutter duration (FR-9.1, FR-9.5) — 는 앱 재시작에서 살아남아, 사용자가 매 중단마다 picker를 다시 거치지 않게 한다. 노출 scale 토큰을 기록하는 이유는 미래 Settings preference가 사용자의 prior 선택을 upgrade 시 덮어쓰지 않고 이어가기 위함이다. (시나리오 8)
...
- **FR-6.1** 계산과 compact하고 훑어보기 좋은 timer 표시는 하나의 primary shooting 화면에 함께 산다: 사용자는 calculator를 떠나지 않고도 실행 중 노출의 남은 시간을 모니터링한다. 모든 timer를 한 번에 살피고 조작하는 full timer management는 그 compact 표시로부터 의도적으로 진입하는 별도의 full-screen 화면이며, shooting 화면에 접혀 들어가 있지 않다. (시나리오 3, 5)
- **FR-6.2** Primary 표면은 기기의 수직 공간에 적응하되 개념 구조를 재배치하지 않는다 — 모든 density에서 같은 요소가 존재, spacing만 변한다. (시나리오 1)
- **FR-6.3** compact dock과 full-screen Timers workspace는 동일한 underlying timer collection의 모든 실행/일시정지/완료/canceled timer를 각각 보여준다 — compact dock은 shooting 화면에서 훑어보기 좋게, full-screen workspace는 timer별 full management action과 함께. 둘 사이의 중간 상태는 out of scope. (시나리오 3, 5)
- **FR-6.4** compact dock은 calculator를 가리지 않으면서 calculator와 공존. 사용자는 timer가 실행 중에도 compact dock을 dismiss 하거나 옮기지 않으면서 calculator 입력을 조정할 수 있다. (시나리오 3, 5; 배치는 디자인 선택, 요구사항 아님.)
...
- **FR-8.2** 각 카메라 슬롯은 자기 calculator 상태를 보존 — 선택 필름과 활성 reciprocity profile, base shutter, ND, 노출 scale, 가장 최근 도출된 reciprocity 결과, 그리고 슬롯의 Target Shutter 상태 (FR-9.1). Digital vs film workflow는 슬롯이 필름을 선택했는지 여부에서 전적으로 derive 되며, 독립적으로 저장되는 슬롯 필드가 아니다. 슬롯은 독립적: 활성 슬롯에서 만든 calculator 변경 — Target Shutter의 enabling, disabling, editing 포함 — 은 비활성 슬롯으로 전파되지 않는다. (Persona 1.3; 시나리오 1, 2)
...
- 자유 텍스트 셔터 입력.
- 사용자 노출 scale selector. 출시 calculator는 1/3-stop scale 에서만 동작; Full / 1/2 / 1/3 stop preference는 미래 Settings 영역로 유보 ([Exposure Calculator spec](../specs/calculator/exposure.md) 참조, 영문).
...
- Cross-device 동기화. 각 폰이 자신의 카탈로그와 영속 상태를 보유. 사용자가 작성한 custom reciprocity profile의 remote sharing도 out of scope; FR-2.9/FR-2.15는 local 작성만 다룬다.
...
- 새 사용자 시나리오 추가, 또는 기존 시나리오 closed (예: 예약되었던 capability가 ship).
...
- Wiki 9601025 — Bottom Sheet UI Architecture (historical; timer workspace shell은 이후 compact dock + full-screen Timers workspace로 이동, `docs/specs/timers/workspace.md` 참조)
- Wiki 8847362 — Floating Timer Dock UI Design (historical 다중 timer 표면 디자인; 동일한 이동으로 superseded)
