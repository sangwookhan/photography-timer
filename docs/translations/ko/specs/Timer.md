...
Timer 의 in-memory 표현은 이 세 상태에 대한 tagged union. 각 상태의 payload 는 그 상태에서 의미 있는 필드만 carry — running 은 expected 종료 시간, paused 는 frozen 남은시간 + paused-at instant, completed 는 기록된 완료 timestamp. 상태들이 record 를 공유하는 nullable sibling 은 없음. 따라서 *invalid* 조합 (예: paused-at instant 를 가진 running timer) 은 표현 자체가 불가능. (PTIMER-118)
...
Timer 가 생성될 때, calculator 의 현재 결과가 timer 의 metadata 로 snapshot: shutter 값, ND stops, film identity (있으면), reciprocity 결과. 후속 calculator 변경은 snapshot 을 변경하지 않는다. 각 timer 는 자체 생성 시점 snapshot 을 carry. (PTIMER-9)
...
남은시간과 status 의 read 는 단일 source 에서 와야 한다. UI layer 는 이들을 독립적으로 snapshot 하거나 re-derive 하지 않는다 — 런타임 모델에서 read. (PTIMER-37)
...
Paused timer 가 resume 될 때, frozen 남은시간이 새 `endDate = now + 남은시간` 의 기반이 된다. 원래 `duration` 은 변하지 않고, `endDate` 만 forward shift. resume 후 즉시 다시 pause 하면 새 남은시간이 freeze. (PTIMER-26)
...
On-disk schema 는 in-memory 표현과 *독립*: 영속화된 record 는 historical flat shape (status discriminator + per-state 필드들) 을 유지하며, encoder 는 런타임이 메모리에서 어떻게 표현하든 그 shape 으로 쓴다. Decoder 는 영속화된 필드들로부터 적절한 tagged-union case 를 재구성. 이 분리로 in-memory form 은 저장 데이터 마이그레이션 없이 진화할 수 있다. (PTIMER-118)

### 3.2 Backward-compatible status 디코딩

영속화된 status 필드는 historical 값을 가진다. Decoder 는 legacy 토큰 `"stopped"` 와 현재 토큰 `"paused"` 모두 받아들이고 같은 상태로 처리. Encoder 는 `"paused"` 만 쓴다. (PTIMER-70)
...
복원은 완료 알림, push notification, 또는 사용자 대면 피드백을 fire 하지 않는다. state recovery 만. (PTIMER-67, PTIMER-70)

### 3.4 Reactivation reconciliation

앱이 포어그라운드로 돌아올 때, 런타임은 실행 중 timer 를 벽시계 대조해 reconcile 하고, 비활성 기간 동안 완료된 경우 상태 update. 완료 알림 (sound, haptic) 은 reactivation 으로 트리거되지 않는다 — 사용자가 인지할 수 있는 포어그라운드 tick 으로만 fire. (PTIMER-67)
...
Timer 가 application active + 포어그라운드일 때 `completed` 로 전환되면, 시스템은 짧은 audio cue + haptic 재생. 각 전환은 정확히 한 cue + 정확히 한 haptic 을 produce. Reactivation 으로 트리거된 완료는 cue 를 produce 하지 않는다. (PTIMER-66)
...
같은 timer identity 에 대한 중복 scheduling 은 발생하지 않는다. (PTIMER-68)

---

## 5. 잠금 화면 표면

시스템은 한 번에 한 *대표* 실행 중 timer 를 잠금 화면에 노출 — 플랫폼의 Live Activity 기능 경유. (PTIMER-69)
...
- **Active 그룹** — running + paused timer 가 한 안정된 정렬 도메인. 정책: **생성 LIFO** — 가장 최근 생성된 timer 가 먼저. running 과 paused 는 그룹 안에서 분리되지 않음 — 둘 다 "active". (PTIMER-50)
...
- **다중 timer 작업의 selection 모델.** Wiki 9601025 가 강한 selection 모델을 의도적으로 deferring. 다중 선택, batch action, 또는 cross-timer 링킹 spec 은 현재 없음.
- **Bottom sheet 의 detent threshold** (compact 98 pt + ND 예약 132 pt; large 560 pt; 92 pt up-drag, 64 pt down-drag) 는 [UI Spec](UI.md) §4 에 문서화. 수치 임계값은 경험적으로 선택 — formal rationale 기록 없음.
...
- **Pause-during-completion race.** "사용자가 런타임 중 완료-evaluation 도중 pause" 에 대한 명시 spec 없음 — 현 동작은 tick ordering 에서 emerge. 명료화 가치 있음.
...
- PTIMER-9 — calculator CTA 와 wired 된 snapshot 기반 timer manager. 다중 timer + date 기반 정확도
- PTIMER-10 — Timer panel UX, 결정적 clock formatting, 남은시간 clamping
- PTIMER-26 — paused 에서 resume (남은시간 보존). time-display 일관성 모델
- PTIMER-29 — 읽기 전용 status 검사, 0 근처에서 안정 표시
- PTIMER-37 — 남은시간/status 의 단일 source of truth. 실행 중 timer 가 redraw 되도록 current-date 스트림 publish
- PTIMER-66 — 포어그라운드 전용 완료 sound/haptic. 완료 알림 중복 제거
- PTIMER-67 — Reactivation reconciliation (state-only, 완료 알림 double-fire 안 함)
- PTIMER-68 — 완료를 위한 local notification 스케줄러 (background / 잠금 화면 전달)
- PTIMER-69 — 대표 실행 중 timer 의 expected 완료 시간을 노출하는 잠금 화면 Live Activity
- PTIMER-70 — 영속화된 timer snapshot + 재시작 복원. `stopped` → `paused` rename + backward-compatible decode
