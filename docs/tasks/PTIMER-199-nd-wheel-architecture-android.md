...
| 4초 정리 타이머 (무장·발화·판단) | 컨트롤러 — 주입 스코프에서 (`ndCleanupScope` = 프로덕션에선 `viewModelScope`) | 구조 변이 시 재무장, 발화 시점 판단 `runNdCleanupIfQuiet` (PTIMER-223 이관) |
...
- **과회전 삭제**: 다른 휠이 활동 중이면 거부, 당긴 휠이 0이
  아니거나 마지막 휠이면 거부. 성공 시 그 휠(identity)만 즉시
  제거하고, 끝에서 장벽을 직접 시도한다 (원칙 4 — 당긴 휠을
  활성 집합에서 빼는 것 자체가 조용함 전이일 수 있다).
...
이 설계의 비동기 표면은 셋뿐이고, 값(데이터)을 나르는 지연
콜백은 없다.

| 경계 | 위치 | 규칙 |
|---|---|---|
| 4초 정리 타이머 | 컨트롤러 — 주입 `ndCleanupScope`(프로덕션 = `ShootingAppViewModel.viewModelScope`)의 Job | 구조 변이(커밋 쓰기·identity 재동기화)에서만 재무장, 4초 후 **눈먼 발화**. 실행 여부는 발화 시점 판단(`runNdCleanupIfQuiet`: 조용함 ∧ 정리 가능), 거부되면 재무장 루프. 표시값 변화는 타이머를 재시작하지 않는다. 구성 변경을 살아남고 소유자와 함께 소멸한다 (PTIMER-223) |
...
- 합계는 배지의 시각 상태와 무관하게 상태 텍스트로 항상 접근
  가능하다 (배지는 비차단 오버레이).
...
- **컨트롤러 = 순수 Kotlin** 이라 상태 동작 전체가 JVM 단위
  ...
  23 케이스): identity 규칙, 세트 커밋 연기·정착 순서(역순
  포함)·30-stop 충돌, A0 shed, C1 거부(0.4 예산 케이스 포함),
  fire-time 정리 판단, 과회전 규칙, 영속 왕복·레거시 폴백·손상
  스택 거부, 슬롯 전환 폐기, 필드 시나리오 2건(제자리 복귀
  릴리스, 빠른 회전+과회전 삭제), 그리고 가상 시간(TestScope)
  으로 구동하는 소유 타이머 3건(유예 후 정리, 손가락 아래 연기,
  구조 변이 재무장).
