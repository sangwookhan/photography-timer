...
/// Whether a successful Filter Stack commit applies the ordinary
/// value-based order or preserves the current spatial order for
/// screen-reader focus continuity (FILTER-A11Y-006).
public enum FilterStackCommitOrderingPolicy: Sendable {
    case automatic
...
public final class CalculatorModel {
    ...
    /// Commits one wheel of the stack, then either applies the agreed
    /// post-commit ordering (FILTER-STACK-005) or preserves the current
    /// complete order while screen-reader ordering is suspended
    /// (FILTER-A11Y-006).
    /// Commits only happen once every wheel has settled (the set-commit
    /// barrier), so sorting here never reorders mid-scroll. Returns the rejection when the domain refuses the
    ...
    /// Reconciles a previously frozen stack to FILTER-STACK-005 once.
    /// Returns whether positions changed; wheel identity follows its
    /// logical wheel through the permutation.
    @discardableResult
    public func reconcileFilterStackOrder() -> Bool {
        let permutation = filterStack.commitSortPermutation(inventory: filterInventory)
        guard permutation != Array(filterStack.wheels.indices) else {
            return false
        }
        applyCommitOrder(to: filterStack)
        return true
    }

    private func applyCommitOrder(to stack: FilterStack) {
        let permutation = stack.commitSortPermutation(inventory: filterInventory)
        filterStack = stack.sortedForCommit(inventory: filterInventory)
    ...
    /// wheels with each live preview substituted — as ONE validated
    /// snapshot, so a timer captures its total, per-wheel summary, and
    /// reference text from the same wheels (FILTER-PERSIST-003). `nil`
    /// when the transient overlay does not form a valid stack (an
    /// over-cap preview the set commit will reject); the committed
    /// stack when no preview is active.
    public var effectiveFilterStack: FilterStack? {
        guard !liveSelections.isEmpty else {
            return filterStack
        }
        let wheels = filterStack.wheels.enumerated().map { offset, wheel -> FilterWheel in
            let wheelID = ndFilterWheelIDs.indices.contains(offset) ? ndFilterWheelIDs[offset] : -1
            guard let live = liveSelections[wheelID] else {
                return wheel
            }
