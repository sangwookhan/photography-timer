        ...
        VStack(alignment: .leading, spacing: style.bodySpacing) {
            ...
            FilterStatusRegionView(
                content: FilterStatusRegionPresenter.content(
                    ...
                    idleSourceSummary: FilterStatusRegionPresenter.sourceSummary(
                        wheels: filterWheels,
...
/// safety come from `TransientStatusRegionController`. The persistent
...
/// pickers and touch centers never move.
        ...
        VStack(alignment: .leading, spacing: 0) {
            if let visible = displayedContent {
                ...
                HStack(alignment: .firstTextBaseline, spacing: 10) {
                    if let leading = leadingText(for: visible) {
                        leading
                ...
                .accessibilityLabel(Text(visible.accessibilityText))
            }
        }
...
private struct NDFilterGroupView: View {
    ...
    /// commit-sort reorder is presented as one whole-row transition
    /// (FILTER-STACK-005), never as columns moving past one another.
    let ndFilterWheelIDs: [Int]
    let rowOptionsForWheel: (Int) -> [FilterWheelRowOption]
    let filterSetColor: (FilterSource) -> FilterSetColor?
    let filterSourceName: (FilterSource) -> String
    ...
    var body: some View {
        ...
        .accessibilityActions {
            if canAddFilterWheel {
                Button(String(localized: "Add filter")) { onAddFilterWheel(selectedFilterSource) }
            }
...
private struct NDWheelView: View {
    ...
    var body: some View {
        ...
        .accessibilityValue(Text(committedDisplay.accessibilityValueText))
