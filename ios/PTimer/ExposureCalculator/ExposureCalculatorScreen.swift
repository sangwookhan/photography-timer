...
struct ExposureCalculatorScreen: View {
    ...
    var body: some View {
        ...
        // An ACTUAL automatic empty-wheel removal is announced exactly
        // once while VoiceOver runs (FILTER-STACK-006); scheduling,
        // deferral, and no-op cleanups publish nothing to announce.
        .onChange(of: viewModel.emptyFilterWheelRemoval) { _, removal in
            guard let removal, UIAccessibility.isVoiceOverRunning else { return }
            UIAccessibility.post(
                notification: .announcement,
                argument: FilterWheelPresenter.emptyWheelRemovalText(count: removal.removedCount)
            )
        }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            VariableSectionView(
                ...
                isFilterInteractionQuiet: pageState.isActive && viewModel.isNDWheelInteractionQuiet,
