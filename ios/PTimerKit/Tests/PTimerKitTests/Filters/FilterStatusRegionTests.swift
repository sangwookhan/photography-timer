...
final class FilterStatusRegionTests: XCTestCase {
    ...
    func testEveryStateIsLeadingTextPlusCompleteTotalWithFullAccessibilityText() {
        ...
        XCTAssertTrue(idle?.primaryText?.contains(longName) ?? false, "The full long name stays in the leading text the view exposes to accessibility even when the row truncates it visually.")
        ...
        XCTAssertEqual(rejected?.secondaryText, "Total 18 stops")
        ...
        XCTAssertEqual(standardOnly?.secondaryText, totalText)
    }
