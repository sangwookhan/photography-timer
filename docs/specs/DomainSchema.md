...
- **ndStop** / **ndStopThirds** — same convention and precedence as §7 / §7.2 for the calculator working context: the fractional-aware `ndStopThirds` field is preferred when present, with `ndStop` treated as a legacy hint. When `ndStack` is present these legacy scalar fields carry the stack's **largest** wheel value, so a build that predates the stack restores the strongest single filter rather than an arbitrary one.
- **ndStack** — optional ordered list of one to four wheel entries, each carrying one ND value; the slot's ND filter stack in display order ([Calculator Spec](Calculator.md) §2.6). Additive Optional per §7.4.2 — pre-stack snapshots omit the key and restore through the legacy scalar fields. Restore validates the list **as a whole**: a wheel count outside 1–4, a value that does not resolve to a shipping-ladder entry, or a total over the 30-stop cap rejects the entire list (no partial recovery, which would silently change the saved combination's sum) and falls back to the legacy scalar. A list that is present but malformed at decode is treated as absent rather than failing the slot. Values are rejected, never clamped.
...
- From a custom **table** profile, the photographer can inspect an **app-derived fitted formula** generated from the table anchors: a narrow power-law fit (`Tc = a × Tmᵖ`, no offset) mapped into the shared formula shape and inheriting the table's no-correction and source-range boundaries. The preview shows the fitted family and parameters, a per-anchor comparison of each source corrected time against the fitted output (percent and stop error), the worst error, and a fit-quality marker that flags poor fits. The fitted formula is **app-derived, not source evidence**; it is derived deterministically from the rule's calculation anchors (never from `sourceEvidence`) and stored nowhere. It is **inspection-only** — the table remains the active calculation, and a fit that would shorten exposure is shown as not usable rather than blocking the table.

- From a saved custom **table** profile, the photographer can **create a separate, editable custom *formula* profile** (PTIMER-180), seeded from the table's app-derived fitted formula. The fitted formula is only the initial seed: once saved, the formula is an independent profile that calculates **solely from its formula parameters** and is edited / deleted independently of the table. The table is never converted, never auto-selected, and gains no hidden runtime formula model — Table interpolation stays its only calculation. A custom formula may carry an optional **film-level reference-table link** (`UserEditableMetadata.referenceTableFilmID`) used only for comparison / error display / provenance — **never calculation**; the formula editor then lists the linked table's metered anchors among its reference points with the table's reference corrected time and the formula-vs-table stop error. Editing the linked table never changes the formula's parameters (errors recompute lazily from the table's current anchors); deleting the linked table leaves the formula valid and hides the reference / error display.

The fitted-formula → editable-formula **seed** is the only table-derived-formula workflow. Directly selecting a table-derived formula as a runtime shooting model, stop-delta / multiplier row input, reference-link relink / unlink management, and remote sharing / sync / inventory management remain **future scope** — they are not part of the custom profile workflows described above.
...
- **Custom table → formula authoring.** User-defined *formula* and *table* profiles are implemented (§13.4); a custom table's anchors can be inspected as an app-derived fitted formula and used to **create a separate, editable custom formula** seeded from that fit, optionally linked back to the table for comparison / error display (`referenceTableFilmID`, display-only) (PTIMER-180). Directly selecting a table-derived formula as a runtime shooting model is intentionally **not** supported; reference-link relink / unlink management remains future scope.
