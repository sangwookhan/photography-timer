...
이는 아날로그 카메라 셔터 다이얼에 표시되는 값. 1/3 stop 입력 모드는 없다.
...
Calculator 의 working context — 선택된 필름 identity + Base Shutter + ND stops — 는 digital + film 양 workflow 에서 영속화 + 재시작 시 복원. 저장된 preset identity 가 어떤 catalog entry 로도 resolve 되지 않거나 숫자 값이 검증 실패면, 시스템은 정의된 default 로 안전하게 fallback (crash 또는 silent drift 아님).
