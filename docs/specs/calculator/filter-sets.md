...
- **FILTER-SET-001** — The user shall be able to create, rename, reorder, edit,
  ...
  Plus wheel while it is visible. While the list is in reorder/delete edit
  mode, the control that exits edit mode shall remain directly visible and
  shall be visually and accessibly distinct from the control that dismisses
  the management surface; ending edit mode shall not require opening an
  overflow menu. A delete action shall remain bound to the targeted Filter
  Set's stable id, the confirmation shall name that Filter Set, and confirming
  shall delete exactly that Filter Set.
...
- **FILTER-STACK-006** — Standard 0 and Filter Set Empty wheels in a multi-wheel
  stack shall follow the existing idle-cleanup contract in `nd-filters.md`.
  For this mixed stack, FILTER-STACK-006 narrows the immediate
  ...
  mounted Record-only item shall not be cleaned up. Automatic cleanup does not
  require a separate assistive-technology Remove action. When it actually
  removes an eligible wheel while a screen reader is active, the result shall
  be announced once with localized wording that identifies an empty filter
  wheel, rather than an item whose contribution happens to be zero.
...
- **FILTER-A11Y-001** — Every wheel shall be one adjustable accessibility
  ...
  Plus control. Automatic cleanup governed by FILTER-STACK-006 need not expose
...
- **FILTER-A11Y-005** — A successful wheel increment or decrement shall update
  ...
  contribution, and the current complete Total in that order. For example, a
  3-stop GND using Apply full value at a 21.6-stop stack total may be announced
  as "GND, Apply full value, 3 stops, Total 21.6 stops". Rapid consecutive
...
1. Register two separate 3-stop items in one Filter Set, select both, and
   ...
   unavailable. In a Filter Set containing 2-, 3-, and 4-stop items, mount the
   4-stop item on one wheel and leave another wheel on a lower selectable row.
   Drag the second wheel toward 4 stops and release with the already-mounted
   4-stop row centered; verify 4 stops never commits and the wheel settles on
   the nearest selectable row just traversed (3 stops, or 2 stops if 3 is also
   unavailable). Verify it does not jump beyond the attempted row. Repeat with
   every traversed candidate unavailable and verify the previous selection
   remains while the actual rejection reason is shown.
2. Combine NiSi ND1000 (10), Lee ND8 plus CPL 1 (registered subtotal
   ...
   a subtotal reorder and verify every wheel remains visible while its stable
   identity, numeric value, label, and source cue move together to the new
   position without changing selection, total, or accessibility focus. Commit
   another reorder before movement completes and verify those same identities
   retarget to only the newest arrangement.
3. Tap Plus and verify the currently displayed source is added exactly once.
   ...
   verify the management surface opens without creating a wheel. With a screen
   reader active, focus Plus and verify both the explicit `Add filter`
   action and the explicit Filter Set management action are discoverable.
   Verify management opens the same surface without adding. Switch
...
4. Select GND 2 as Record only and observe a mounted item with contribution 0;
   ...
   camera or a running timer. In a multi-wheel stack, leave another wheel at
   Empty until idle cleanup removes it. With a screen reader active, verify the
   actual removal produces exactly one localized announcement identifying an
   empty filter wheel; verify the mounted Record-only item remains and is not
   described as removed. Repeat with an eligible empty wheel that becomes
   immediately unusable at the 30-stop limit and verify immediate cleanup
   produces the same single announcement only after the wheel is actually
   removed. Verify scheduling, deferral, cancellation, and cleanup attempts
   that remove nothing produce no removal announcement.
...
7. Rename, recolor, and reorder a Filter Set; verify stable ids, active stacks,
   accessible names, and captured timer summaries remain correct. Enter the
   management list's reorder/delete edit mode and verify its exit control stays
   directly visible and distinct from the management-surface close action.
   Start deletion from each of two differently named Filter Sets in turn;
   verify the confirmation names the focused Filter Set and confirming deletes
   exactly that Filter Set while the other remains unchanged.
...
11. With Filter Set and Standard sources mixed, verify the idle status region
   ...
   summary and transient detail while the complete Total remains a separate
   focusable target that can be reached directly without hearing the source
   summary first.
   ...
   total layout before returning to the idle presentation. While a Filter Set
   wheel moves or is adjusted, verify its full physical-item name is visible in
   the leading detail and exposed to accessibility even though picker rows stay
   numeric-only. Repeat the palette and summary checks in light and dark
   appearances.
...
15. Create a Filter Set containing ND 3, ND 6, ND 10, and CPL. Mount ND 10 in
   ...
   candidate in that direction is available because candidates were skipped;
   verify the current selection and total remain unchanged and the actual
   unavailability reason is announced. Then adjust from an ordinary end with no
   rejected candidate and verify a localized boundary reason is announced.
16. Focus each wheel with VoiceOver and verify its stable label identifies only
   ...
   Verify each successful adjustment announces the newly committed item or
   Empty, its type or mode, its current value or contribution, followed by the
   current complete Total. For a GND using Apply full value, verify the spoken
   result includes the mode, the GND's value, and the updated stack Total.
   Rapidly adjust through at least four available rows and stop; intermediate
   speech may be interrupted, but the final committed value and current Total
   shall be heard exactly once. Verify Total also remains a separate focusable
   target in the status region that can be reached directly without first
   listening through the source summary or item detail. Repeat a rejected and a
   boundary adjustment; verify only the localized reason is announced and the
   value, stack, and total remain unchanged. Repeat in English and Korean.
