...
2. **Official table log-log** (PTIMER-159) — 제조사 reciprocity *table*을 출판 anchor 사이 piecewise log-log interpolation으로 보정 노출로 변환하는 `tableInterpolation` rule, 그 anchor를 보존하는 `sourceEvidence` row 동반. Rule 자체가 무보정 band + source range를 소유. 계산은 band 아래에서 `officialThresholdNoCorrection`, 출판 범위 안에서 `tableLogLogDerived`, 마지막 anchor 위에서 (마지막 log-log 세그먼트로 외삽된 numeric continuation을 carry하는) `unsupportedOutOfPolicyRange` 산출. Fomapan 100 Classic이 이 shape의 현 launch profile; 그 app-derived power-law 공식은 비-default alternate로 보존 (§13.3).
