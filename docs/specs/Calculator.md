...
- **User-defined table input.** User-defined *formula* and *table* profiles are implemented and evaluated through the shared guarded formula path and the log-log table path ([DomainSchema Spec](DomainSchema.md) §13.4). A custom table's anchors can also be inspected as an app-derived fitted formula (point fitting), and used to **create a separate, editable custom formula** seeded from that fit — an independent profile that calculates only from its own parameters and is edited / deleted independently of the table, optionally linked back to the table for comparison / error display only ([DomainSchema Spec](DomainSchema.md) §13.4; PTIMER-180). Directly selecting a table-derived formula as a runtime shooting model in place of the table is intentionally not supported.
