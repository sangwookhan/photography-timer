...
5. timer를 시작한 뒤 중간에 pause 하면 예정된 completion notification이 취소되고, 원래 completion 시각이 지나도 알림이 오지 않는지 확인한다.
6. pause 한 timer를 resume 하면 새 endDate 기준으로 notification이 다시 예약되고, 새 completion 시각에만 1회 알림이 오는지 확인한다.
...
- Verify paused, removed, and cleared timers cancel pending completion notifications.
