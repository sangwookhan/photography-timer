...
- **User-defined table input.** User-defined *formula* and *table* profiles are implemented and evaluated through the shared guarded formula path and the log-log table path ([DomainSchema Spec](DomainSchema.md) §13.4). A custom table's anchors can also be inspected as an app-derived fitted formula (point fitting). Selecting that fitted formula as the active custom calculation model — in place of the table — remains future scope and would need its own selection and persistence design.
