...
- **ND picker** — **integer stops in the closed range `[0, 30]`** plus three fixed fractional presets (`6.6` = ND100, `7.6` = ND200, `16.6` = ND100k), spliced in numeric order (`… 6, 6.6, 7, 7.6, 8, … 16, 16.6, 17 …`). One-third-stop applies to the Base Shutter ladder only ([Calculator Spec](Calculator.md) §1.4); the ND picker stays whole-stop apart from those presets because real-world fixed ND filters are sold in whole-stop strengths. One-third-stop rows such as `7 1/3` or `7 2/3` shall not exist in the option set. The presets are permanent — present regardless of the active ND notation (Stops / OD / ND number) — and stops stay canonical (see [Calculator Spec](Calculator.md) §2.2).

#### 2.2.1 ND filter stack presentation

The ND area presents the filter stack ([Calculator Spec](Calculator.md) §2.6) as **one to four side-by-side wheels**, each an independent picker over its budget-truncated ladder:

- **Add affordance.** A slim, deliberately understated add control sits at the trailing edge of the wheel row. Its **presence** follows the add-availability rule on committed values; while a wheel interaction is in flight it stays in the layout (so wheels never resize under a moving finger) but is disabled and visually dimmed. There are no menus anywhere in the group — adding is this control, removal is self-cleaning plus the gestures and accessibility actions below.
- **Reorder as movement.** The post-commit descending sort animates as wheels sliding across each other; a wheel visibly keeps its identity through the move. Wheel additions and removals animate as a width collapse/expansion of the affected wheel.
- **Transient total overlay.** While two or more wheels are stacked, a small non-blocking badge overlays the wheel row showing the effective sum, always in stops (`Total N stops`, with a `Maximum` marker at the 30-stop cap). It appears (or re-shows) on any effective change — updating live during a scroll — and fades after a short idle, slightly longer right after a wheel was added. Touches always pass through it, and the current total stays available to assistive technologies regardless of the badge's visual state.
- **Zero-wheel removal gestures.** Pulling a 0-stop wheel past its zero end by at least one row height and releasing removes exactly that wheel, with a live, damped push-out preview during the pull. Untouched 0-stop wheels in a multi-wheel stack self-clean after the idle interval ([Calculator Spec](Calculator.md) §2.6).
- **Accessibility.** Each wheel is an adjustable element labeled with its position in the stack. *Add filter* and *Remove empty filter* are exposed as custom accessibility actions, registered only while the corresponding operation is actually available so assistive users never surface a dead command.
- **Density.** From three wheels the group may tighten typography and the Base Shutter column may narrow so every wheel's labels stay legible; values shall not be hidden or elided as wheel count grows.
