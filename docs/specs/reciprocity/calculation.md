...
- **RECIP-CONF-001** — Every result shall map to one of these user-facing
  ...
  - **Custom formula** / **Custom table** — the same two calculation
    ...
    **Formula-derived** / **Table-derived** — the badge text itself does
    ...
    authority.
  - **Beyond source range** — a numeric continuation past a profile's
    supported/source boundary, covering a converted formula or a
    table-log-log model past its boundary, and — unconditionally,
    regardless of boundary shape — any user-defined custom profile's
    out-of-range numeric continuation.
