...
Table-vs-fitted-formula **model selection** (choosing the fitted formula as the active shooting calculation), its persistence and timer identity, stop-delta / multiplier row input, and remote sharing / sync / inventory management remain **future scope** — they are not part of the custom profile workflows described above.
...
- **Custom table model selection.** User-defined *formula* and *table* profiles are implemented (§13.4), and a custom table's anchors can be inspected as an app-derived fitted formula. Selecting that fitted formula as the active shooting calculation — and persisting the table-vs-formula choice and its timer identity — remains future scope; the selection policy and its persistence are not specified.
