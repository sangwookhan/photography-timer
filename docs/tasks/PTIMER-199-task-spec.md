...
- A1 — IDLE CLEANUP: while at least one cleanable 0-stop wheel
  exists, 4 seconds of no ND-area activity trigger automatic
  cleanup. Wheel origin is irrelevant.
...
- A3 — FIRE-TIME JUDGMENT (architecture v2; replaces the earlier
  ...
  period. There is no cancellation path, so no interaction
  bookkeeping (and no touch-start signal) is required; a wheel
  still never vanishes under a finger or mid-scroll. Notation
