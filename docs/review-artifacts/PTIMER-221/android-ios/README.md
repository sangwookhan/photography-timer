...
| `29-large-text-narrow-android-only` | **Android only, not parity evidence.** The shooting card at 360dp with the maximum font scale and four wheels, in English (`android`) and Korean (`-b`), plus 411dp at default text for contrast (`-c`). Recorded because that state is where the Android layout had to change: the notation options and the management entry now span the whole card so each option owns a 48dp target, and the Base Shutter caption wraps rather than any control shrinking. iOS has no matching state — its sizing model and its Dynamic Type steps are different, so there is nothing to pair it with. |
