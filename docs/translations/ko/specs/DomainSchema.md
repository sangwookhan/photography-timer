...
본 문서는 도메인의 **모양과 의미** 를 명세 — 어떤 필드가 존재하고, 무엇을 의미하며, 어떤 invariant 가 유지되는가. 인코딩 형식 (현재 JSON, union 타입에 `kind` discriminator) 은 §10 에 기록된 직렬화 디테일; spec 본문은 플랫폼 + 직렬화기 중립.
...
- **productionStatus** — `current`, `discontinued`, `unknown` 중 하나. Launch dataset entry 는 `current` 가짐. (PTIMER-86)
- **profiles** — [reciprocity profile](#3-reciprocity-profile) 배열. Launch dataset 은 identity 당 정확히 한 entry.

### 2.2 옵션 필드

- **brandLabel** — 필름이 현재 판매되는 secondary 브랜드 문자열 (예: 제조사-rebranded variant).
...
Preset launch entry 는 사용자 metadata 를 carry 하지 않는다. (PTIMER-86)
...
- **kind** — `manufacturer_published`, `manufacturer_secondary` (예: 라이센시를 통해 재발행된 데이터 시트), `vendor_published`, `community_field_tested`, `historical_archive` 중 하나. Launch dataset 은 `manufacturer_published` 만 사용. (PTIMER-86)
- **authority** — `official`, `field_tested`, `anecdotal` 중 하나. Launch dataset 은 `official` 만 사용.
- **confidence** — `unknown`, `high`, `medium`, `low` 중 하나. 생략의 default 는 `unknown`. Launch dataset 은 일반적으로 `high` 사용.
...
- **formula** — 현재 **지수 power** 형태만 정의: `T_c = T_m^P` + 옵션 계수 + offset (`T_c = coefficient × T_m^exponent + offsetSeconds`). 구조는 투명성을 위해 source 의 출판된 등식 문자열 포함.
...
`correctedExposure` 의 존재는 form (Quantified vs Advisory-only/Unsupported) 으로 구조적 결정 — form 이 보정 노출을 claim 하면서 payload 가 없거나 (그 반대) 인 결과는 구성상 표현 불가능. (PTIMER-90, PTIMER-118)
...
bundled launch 카탈로그는 런타임이 받아들이기 전 다음 검사 통과해야. (PTIMER-86, PTIMER-96)
...
예: Kodak PORTRA 400 의 unofficial practical 공식 `T_c = T_m^1.34` 가 launch 카탈로그 외부에 PORTRA 400 의 official threshold-only profile 의 secondary 대안으로 bundle. (PTIMER-112)
...
4. 보정 노출을 claim 하면서 값을 carry 하지 않거나, 보정 노출 값을 carry 하면서 claim 하지 않는 계산 결과 허용. 모순 페어링은 결과의 form 으로 표현 불가능. (PTIMER-90, PTIMER-118)
...
- **현상-시간 조정.** Tri-X 스타일 "dev −10%" 가 wiki 안내의 일부 + commit-locked 샘플 데이터에 embedded but first-class schema 필드 없음.
...
- PTIMER-17 — 도메인 모델에 정렬된 검증 샘플; Velvia 50 + Tri-X 표 경계의 round-trip-safe profile
- PTIMER-86 — Launch preset 정책: 필름 identity 당 한 primary preset profile ship; archival / unofficial 대안은 launch bundle 외부에 유지
- PTIMER-90 — 계산 결과 contract: 도메인 shape lock; 불가능한 결과 조합은 decode 시 거부
- PTIMER-92 — Bundled launch preset 카탈로그에 film mode binding
- PTIMER-96 — 카탈로그를 명시적 decode 진단과 함께 bundled JSON 리소스로 외부화
- PTIMER-112 — 비-launch unofficial profile 을 launch 카탈로그 외부에 bundle (PORTRA 400 unofficial practical 공식 `T_c = T_m^1.34`); 같은 도메인 shape, 정직한 provenance (`unofficial` authority), secondary 대안으로 선택 가능
