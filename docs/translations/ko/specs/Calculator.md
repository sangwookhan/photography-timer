...
모든 노출 조정 math 는 **stop 공간** (밑이 2 인 로그) 에서 수행. 한 stop = 빛이 두 배 변함. 내부적으로 ND 값은 정수 stop, shutter target 은 다음으로 계산:

```
output_seconds = base_seconds × 2^stops
```

Factor 형태로 들어오는 입력 (예: ND 64×) 은 calculator 진입 전 stop 으로 변환되어야 한다.

### 2.2 ND 입력 범위

ND stops 는 **[0, 30]** 닫힌 구간의 정수. Picker 가 이 범위를 제시; 범위 밖 값은 표현 불가능.

### 2.3 Base shutter 값

Base Shutter picker 는 **19 풀스톱 표준 속도** 를 제시:
...
이는 아날로그 카메라가 일반적으로 표시하는 값. 1/3 stop 입력 모드는 없다.

### 2.4 Snap-to-full-stop 출력 규칙

시스템이 출력 셔터를 계산할 때, 표기 관습성을 위해 같은 stop-aligned 참조 스케일에서 도출된 값을 보고:
...
ND 조정은 forward 또는 reverse 로 실행 가능:
...
5. **Formula** — profile 이 지수 공식 `T_c = T_m^P` (또는 등가) 를 정의하면 적용해 basis = `formula_derived` 반환. Formula 평가는 generic "unsupported" fallback 전에 실행 — 공식을 가진 필름이 긴 노출에서도 quantified 유지되도록. (PTIMER-101)
...
결과 form 과 calculation basis 는 함께 묶임: `quantified` ↔ `exact_table_point`, `interpolated_within_table`, `extrapolated_beyond_table`, `official_threshold_no_correction`, 또는 `formula_derived`; `advisory_only_beyond_official_range` ↔ `advisoryOnly`; `unsupported_out_of_policy_range` ↔ `unsupported`. 이 페어링은 *구조적* — 런타임 검사가 아닌 컴파일 타임에 강제. 따라서 결과가 숫자 보정값을 claim 하면서 그것을 omit (또는 그 반대) 하는 것이 불가능. 이는 모순된 디코딩 결과를 거부하던 이전 런타임 invariant 를 supersede. (PTIMER-90)
...
영속화는 backward-compatible: 이전에 저장된 flat 필드 layout (측광, 보정, 명시적 returned-time 플래그, metadata block) 결과는 그에 대응하는 form 으로 손실 없이 계속 디코드. 새로 쓰는 결과는 3-form 표현 사용. ([DomainSchema Spec](DomainSchema.md) §6 참조.)
...
- **Extrapolated** — basis = `extrapolated_beyond_table`. extrapolated 와 estimated 는 distinct 한 카테고리로 표시; extrapolated 는 더 강한 low-confidence signal 을 carry. (PTIMER-89, PTIMER-102)
- **Advisory-only** — basis = `advisory_only_beyond_official_range` 또는 임계값 band 너머의 threshold 무보정. UI 는 숫자 대신 차분한 설명 텍스트 표시; 값을 fabricate 하지 않는다. (PTIMER-93, PTIMER-94)
...
Timer 는 **Output Shutter** (digital workflow) 또는 **Corrected Exposure** (film workflow) 에서 생성. 시스템은 비-quantified 결과에서 timer 를 시작하지 않는다 — 보정 노출이 advisory-only 또는 unsupported 일 때 Film 모드 timer-start affordance 는 비활성화되며, 사용자는 입력을 변경하거나 ND-조정된 셔터로 명시적으로 진행하도록 안내. (PTIMER-94, PTIMER-99)
...
Calculator 의 working context — 선택된 필름 identity + Base Shutter + ND stops — 는 digital + film 양 workflow 에서 영속화 + 재시작 시 복원. 저장된 preset identity 가 어떤 catalog entry 로도 resolve 되지 않거나 숫자 값이 검증 실패면, 시스템은 정의된 default 로 안전하게 fallback (crash 또는 silent drift 아님). (PTIMER-97)
...
- **외삽 cap.** 외삽 ceiling 은 제거됨 (PTIMER-102) but 제조사 stop signal 너머 대체 상한이 없음. Profile 에 stop signal 이 없으면 외삽은 unbounded 로 계속 — 현 contract 이지만 refinement 가치 있음.
- **사용자 정의 필름 schema.** Wiki 15138817 이 검증 요구사항으로 list — 데이터 모델과 UX 는 명시 안 됨.
- **다중 profile 필름.** 일부 필름은 다중 official profile 가질 수 있음 (다른 현상액, push/pull). 선택 규칙은 아직 정의 안 됨; 현 launch 정책은 필름 identity 당 한 primary profile (PTIMER-86) ship.
...
- PTIMER-19 — Stops 를 정규화 노출 단위로 (multiplicative factor 를 stop 기반 math 로 대체)
- PTIMER-20, 21, 22, 64 — Picker 기반 셔터/ND 입력, 30 초 전환 규칙
- PTIMER-86, 96 — Launch preset catalog 정책: 필름 identity 당 한 primary preset profile, bundled JSON 리소스로 외부화
- PTIMER-89, 90 — Reciprocity 정책 evaluator + 카테고리별 construction path + contract invariant
- PTIMER-92, 94, 95, 98 — Film mode binding, quantified-only timer start, advisory wording, threshold 무보정 정렬
- PTIMER-97 — 재시작 가로질러 working context 복원 (preset identity + base shutter + ND), 잘못된 저장 값에 대한 안전한 fallback
- PTIMER-99, 100 — Reciprocity 상세 표면 (graph, summary)
- PTIMER-101 — Formula reciprocity 규칙이 generic unsupported fallback 전에 평가
- PTIMER-102 — Generic table 외삽 ceiling 제거. quantified 외삽은 제조사 stop signal 가 차단할 때까지 계속
- PTIMER-109 — Threshold-to-table 갭: 처음 두 quantified 표 점에서 하향 외삽
- PTIMER-110 — Film picker 시트 모델
