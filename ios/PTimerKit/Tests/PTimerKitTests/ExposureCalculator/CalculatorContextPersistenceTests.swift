...
final class CalculatorContextPersistenceTests: XCTestCase {
    ...
    func testRelaunchRestoresTimerCardIdentityMetadataForMultipleTimers() throws {
        ...
        let initialTimerManager = RuntimeBackedTimerManaging(
            tickInterval: 60,
        ...
        let relaunchedTimerManager = RuntimeBackedTimerManaging(
            tickInterval: 60,
            dateProvider: { currentDate },
            persistenceStore: timerStore
        )
...
/// File-scoped duplicate of the app test-support stub (it lives in the
/// app target's `TimerManagerTestSupport`); duplicated `private` here so
/// the moved suite stays off-simulator.
